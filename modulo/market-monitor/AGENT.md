...
| `js/data.js` | `D` (illustrative dataset: mercados, categorias, productos, competidores), `FICHAS` (one handwritten cadena ficha per product: prod/costos/post/com/proc/derivados — partidas and canales each sum 100), `QAS` (2–3 handwritten FAQ pairs per product), `FUENTES` (retail observation source URL per product id — SupermercadosRD group pages; 40 of 59 products) + `H` (helpers: fmt, tendencia, media, índices, `ficha(id)`, `fuente(id)`). |
...
- **Router**: `route()` (js/app.js) parses `location.hash` → calls a view builder → injects into `#view` → `bindView()`. Routes: `#/mercado` (+ `/categoria/:id`, `/producto/:id` — the producto page carries nine identifiable sections: `#sec-resumen/evolucion/plazas/produccion/cadena/proceso/derivados/preguntas/referencia`; **editorial identity per the Método pattern** — each section opens with a `figcap--section` header **whose tag is the section name (matching its pagenav pill)**, figure numbers live in the caption, and its body flows directly on the canvas: one `data-q` part per body (`produccion`, `costos`, `postcosecha`, `comercial`, `proceso`, `derivados`, `preguntas`) built from `H.ficha(p.id)` / `QAS[p.id]`; Cadena carries one header plus three `.figcap--block` sub-captions (Figs. 4–6); no per-figure cards and no scattered `.source` — sources consolidate in `#sec-referencia` (`.refs` list: plaza network + SupermercadosRD group when `H.fuente(p.id)` exists); only Fig. 1 keeps a `.figbox--mod` card (chart); the page head carries a `.chip--link` to the product's SupermercadosRD group when `H.fuente(p.id)` exists), `#/internacional` (sets `scope = "internacional"` and renders the same `vBoard` — no separate view), `#/catalogo` (`vCatalogo`: card grid of all products with price-history sparklines; **a data view** — it is included in `dataScope` so geoseg/winchip/scrollprog render, cards respect scope, plaza coverage and the toolbar's own filters), `#/benchmark`, `#/herramientas` (+ `/coleccion/{consultar|gestionar}`; `#/coleccion/...` is a legacy alias), `#/metodo` (+ `/elemento/:id`; the page carries a «Fuentes de observación» section — `metodoFuentesHTML()` inverts `FUENTES` into a group × categoría × productos table), `#/cuenta`, `#/docs`, `#/acerca`.
- **Pagenav scrollspy** (`bindPagenav`): active pill **computed on scroll** (passive listener + resize + initial call, handler removed/rebound per route) — the last section whose top crosses the sticky line (pagenav bottom + margin), first pill above it. No IntersectionObserver: short sections at the document end would never cross an observer band. For the last section to reach the line, the page tail carries height — `#sec-referencia` gets `min-height: calc(100vh − topbar − pagenav − 5.5rem)` — so every pill on the producto page can activate.
- **Catálogo**: session state `cstate = { q, cat, trend, sort, range }` (not persisted). The toolbar is a `.tmodule` with `.tablebar`; the grid is a stable container `#catalogo-grid` (async via `qMap.cards` on first render). Text input swaps the grid in place (`catalogoSwapGrid`) and updates `#cbar-meta`; chips/sort/range/clear re-render the whole module through `updateCatalogo()` + `bindCatalogo()` (bound per-`bindView`). Card sparks: full national serie, subset → aggregate of synthesized `plazaSerie` over `activeMercados()`, Internacional → national serie + `intl` chip + prima; `cstate.range` slices the window (1M → last 2 points, 6M → last 6). Prices: subset → `H.mediaMercados`, else `H.actual`.
